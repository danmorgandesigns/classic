async function loadLatestPost() {
  ...
  try {
    ...
    const postRes = await fetch("blog-posts/" + latest.filename);
...
function loadBlogSafely() {
  if (window._hasLoadedBlogContent) return;
  window._hasLoadedBlogContent = true;
  loadLatestPost();
}
...
if (document.readyState === "complete" || document.readyState === "interactive") {
  ...
  document.addEventListener("DOMContentLoaded", loadBlogSafely, { once: true });
  window.addEventListener("pageshow", loadBlogSafely, { once: true });
}
